...
FROM php:${PHP_VERSION}-fpm-${DEBIAN_VERSION} AS opendxp_php_min
...
COPY files/conf/php/php.ini /usr/local/etc/php/conf.d/20-opendxp.ini
...
FROM opendxp_php_min AS opendxp_php_default
...
FROM opendxp_php_default AS opendxp_php_max
...
FROM opendxp_php_default AS opendxp_php_debug
...
FROM opendxp_php_default AS opendxp_php_supervisord
